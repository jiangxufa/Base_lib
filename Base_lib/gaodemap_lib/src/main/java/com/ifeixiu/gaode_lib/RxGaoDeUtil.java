...
public class RxGaoDeUtil {

    public static Observable<AMapLocation> fetchLocation() {
        return Observable.create(new ObservableOnSubscribe<AMapLocation>() {
            ...
            public void subscribe(final ObservableEmitter<AMapLocation> e) throws Exception {
                final GaodeLocationUtil instance = GaodeLocationUtil.getInstance();
                instance.registerLocationObserver(new GaodeLocationUtil.LocationObserver() {
                    @Override
                    public void onLocationSucess(AMapLocation amapLocation) {
                        instance.unregisterLocationObserver(this);
                        if (e.isDisposed()) {
                            return;
                        }
                    ...
                    public void onLocationFail(AMapLocation amapLocation) {
                        instance.unregisterLocationObserver(this);
                        e.onError(new Exception(amapLocation.getErrorInfo()));
                    }
                });
    ...
    private static final String Kilometer = "\u516c\u91cc";// "公里";
    private static final String Meter = "\u7c73";// "米";
                        ...
                        if (!e.isDisposed()) {
                            if (errorCode == AMapException.CODE_AMAP_SUCCESS) {
                                if (driveRouteResult != null && driveRouteResult.getPaths() != null) {
                                    ...
                                    e.onNext(new String[]{getFriendlyLength(dis), getFriendlyTime(dur)});
                                    e.onComplete();
                                ...
                                e.onError(new Exception("高德错误代码为:" + errorCode + ""));
                            }
                        }
                    }
